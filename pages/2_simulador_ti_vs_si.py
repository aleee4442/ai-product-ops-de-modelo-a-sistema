...
with st.sidebar:
    ...
    tokens_por_ticket = st.slider("Tokens por ticket (estimado)", 50, 6000, 800, step=50)
    ...
    aht_min = st.slider("AHT (min) por ticket que rebota", 1.0, 60.0, 15.0, step=1.0)
...
if fcr < 0.65 and r["coste_operativo"] > r["coste_ti"]:
    st.error("ALERTA: estás ahorrando en TI, pero el retrabajo humano domina el coste total (SI).")
