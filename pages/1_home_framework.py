...
with st.expander("Definiciones rápidas (para discusión en clase)", expanded=True):
    st.markdown("""
...
with colB:
    ...
    fig = go.Figure(
        data=[go.Scatterpolar(r=values + [values[0]], theta=categories + [categories[0]], fill="toself")]
    )
    fig.update_layout(
...
st.info("Objetivo didáctico: antes de “tocar prompts”, forzar decisiones de sistema (roles, flujos, métricas y políticas).")